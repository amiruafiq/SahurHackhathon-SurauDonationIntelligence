...
Surau/Mosque Finance Dashboard — Hackathon 2026
"""
...
def parse_description(desc):
    ...
    sender_name = ""
    name_patterns = [
        r"DuitNow QR POS\s+([A-Z][A-Z\s\']+?)\s+\d{8}",
        r"DuitNow Transfer\s+([A-Z][A-Z\s\']+?)\s+(?:Infaq|Sedekah|Sumbangan|Fund Transfer|Akhirat)",
        r"IBG TRANSFER TO CA\s+([A-Z][A-Z\s\']+?)\s+(?:Sumbangan|Sedekah|\d)",
        r"MB SA TRF CA 3RD PTY\s+\w+\s+([A-Z][A-Z\s\']+?)\s+EFO",
    ]
    ...
    category = "Donation"
    category_map = {
        "infaq": "Infaq", "sedekah": "Sedekah",
        "sumbangan": "Sumbangan", "akhirat": "Sedekah",
        "cash deposit": "Cash Deposit", "qr payment": "QR Payment"
    }
    ...
    return txn_type, sender_name, ref_code, category
...
def extract_transactions(pdf_path):
    ...
    for row in all_rows:
        if DATE_RE.match(row["date_str"]):
            ...
            for col in ["debit_str", "credit_str"]:
    ...
    for row in merged:
        ...
        txn_type, sender_name, ref_code, category = parse_description(desc)
...
if __name__ == "__main__":
    ...
    if df.empty:
        print("❌ No transactions extracted. Check PDF path or layout.")
    else:
        ...
        print(f"\n--- Sample (5 rows) ---")
        print(df[["date","txn_type","sender_name","category","credit_rm","direction"]].head(5).to_string())
